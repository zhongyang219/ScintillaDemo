...
void ScintillaBase::ContextMenu(Point pt) {
	if (displayPopupMenu) {
		...
		AddToPopUp(_T("撤销"), idcmdUndo, writable && pdoc->CanUndo());
		AddToPopUp(_T("重做"), idcmdRedo, writable && pdoc->CanRedo());
		AddToPopUp(_T(""));
		AddToPopUp(_T("剪切"), idcmdCut, writable && currentPos != anchor);
		AddToPopUp(_T("复制"), idcmdCopy, currentPos != anchor);
		AddToPopUp(_T("粘贴"), idcmdPaste, writable && WndProc(SCI_CANPASTE, 0, 0));
		AddToPopUp(_T("删除"), idcmdDelete, writable && currentPos != anchor);
		AddToPopUp(_T(""));
		AddToPopUp(_T("全选"), idcmdSelectAll);
		popup.Show(pt, wMain);
	}
}
